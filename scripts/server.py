...
def generate(prompt, W, H, C, scale, steps, sampler_type, seed):
  try:
    if (seed == 0):
...
# Warm up with throw-away work
generate("lorem ipsum", default_W, default_H, default_C, default_scale, 20, 'plms', 0)
...
while True:
  for message in queue.receive_messages(MaxNumberOfMessages=1, WaitTimeSeconds=20):
    ...
    image = generate(prompt, W, H, C, scale, steps, sampler_type, seed)
    s3_object = s3.Object(s3_bucket, f'sd/{jid}.png')
    s3_object.put(Body=image.getvalue(), ContentType='image/png')
    message.delete()
